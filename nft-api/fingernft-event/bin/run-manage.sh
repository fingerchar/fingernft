...
readonly APP_MAIN_CLASS="fingernft-event-0.1.0-exec.jar"
readonly APPLICATION_CONF="$APP_HOME/application.yml"
...
readonly JAVA_RUN="-Dlogs.home=$LOGS_HOME  -Dspring.pid.file=$PID_FILE -Dspring.pid.fail-on-write-error=true"
readonly JAVA_OPTS="-server -Xms640m -Xmx640m -XX:PermSize=128M -XX:MaxPermSize=128M $JAVA_RUN"
...
function start() {
   ...
   then
      ...
      JAVA_CMD="nohup $JAVA $JAVA_OPTS -jar $LIB_HOME/$APP_MAIN_CLASS --spring.config.location=$APPLICATION_CONF > /dev/null 2>&1 &"
